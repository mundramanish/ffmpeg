...
class CameraVC: UIViewController, AVCaptureFileOutputRecordingDelegate {
    ...
    // Added by mainsh
    var isCameraAudioUse: Bool = true
    var playSoundBlock: (() -> Void)?
    var executeFFMPEGBlock: ((String) -> Void)?
    var isHVView: Bool?
    ...
    override func viewDidLoad() {
        ...
        self.captureModeControl.selectedSegmentIndex = 1
        self.captureModeControl.sendActions(for: UIControl.Event.valueChanged)
            ...
            if session.canAddInput(videoDeviceInput) {
                ...
                DispatchQueue.main.async {
                    /*
                     ...
                     handled by CameraVC.viewWillTransition(to:with:).
                     */
...
//                session.addInput(audioDeviceInput)
                if self.isCameraAudioUse {
                    session.addInput(audioDeviceInput) // Manish
                } else {
                    // We do not required camera audio, so skip it
                }
            } else {
            ...
            sessionQueue.async {
                ...
                if self.session.canAddOutput(movieFileOutput) {
                    ...
                    if self.selectedMovieMode10BitDeviceFormat != nil {
                        DispatchQueue.main.async {
                            self.HDRVideoModeButton.isHidden = true
    ...
    @IBAction func toggleMovieRecording(_ recordButton: UIButton) {
        ...
        sessionQueue.async {
            if !movieFileOutput.isRecording {
                if let block = self.playSoundBlock {
                    block()
                }
...
//            }
            ...
            if let block = executeFFMPEGBlock {
                block(outputFileURL.path)
